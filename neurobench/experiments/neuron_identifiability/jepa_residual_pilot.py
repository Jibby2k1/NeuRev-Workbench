"""Bounded conditional-JEPA pixel-residual screen for ``NREV-EXP-0029``.
...
def normalize_raw_movie_once(movie: np.ndarray, normalization: RobustNormalization) -> np.ndarray:
    """The single raw-to-model-units boundary used only for raw fixtures."""
...
def _normalization_contract_check(
    ...
    return {
        ...
        "raw_fixture_normalization_application_count": 1,
...
def _guarded_train_decoder_arms(
    ...
    *,
    ...
    """Make the geometry gate structurally precede the first optimizer step."""
...
def _prediction_numpy(output: Any) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    if background.ndim != 5 or int(background.shape[0]) != 1 or int(background.shape[1]) != 1:
        raise JEPAResidualPilotError("background sweep must return [1,1,T,H,W]")
...
def _report(summary: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        f"Source-on macro recall was `{recall.get(JEPA_RESIDUAL_METHOD)}` for the JEPA conditional residual, `{recall.get(RANDOM_RESIDUAL_METHOD)}` for the matched random-provider residual, and `{recall.get(RAW_METHOD)}` for the frozen normalized-video handcrafted comparator. The paired JEPA-residual-minus-raw effect was `{effect['observed_mean']}` with a hierarchical 95% interval of `{effect['confidence_interval_95']}`; random-residual-minus-raw was `{random_effect['observed_mean']}` with `{random_effect['confidence_interval_95']}`, and JEPA-residual-minus-random was `{specificity_effect['observed_mean']}` with `{specificity_effect['confidence_interval_95']}`. Source counts were averaged within recording/window/injection-seed clusters before resampling.\n\n"
        f"Per-recording directions for all three comparisons were `{json.dumps(directions, sort_keys=True)}`. The raw-HC anchor matched EXP-0028 exactly for all `{summary['raw_hc_cross_run_regression']['execution_fixture_count']}` executed fixtures.\n\n"
...
def run_jepa_residual_pilot(
    ...
    *,
    ...
    try:
        ...
        _heartbeat(partial, "loading_verified_parent")
        checkpoint_payload = _load_strict_checkpoint_payload(
            paths["checkpoint"],
            expected_sha256=config["upstream_screen"]["checkpoint"]["sha256"],
        )
        # Import only after the complete parent integrity pass so no checkpoint
        # loader can run before the fail-closed hash boundary above.
        ...
        if not isinstance(checkpoint_model, nn.Module):
            raise JEPAResidualPilotError("loaded provider lacks its strict checkpoint module")
        if tensor_mapping_sha256(checkpoint_model.state_dict()) != expected_tensor_hash:
            raise JEPAResidualPilotError("loaded JEPA weights differ from checkpoint state")
        ...
        if not isinstance(random_checkpoint_model, nn.Module) or tensor_mapping_sha256(
            ...
            raise JEPAResidualPilotError("loaded random-control weights differ from checkpoint")
        if any(parameter.requires_grad for parameter in random_provider_nn.parameters()):
            raise JEPAResidualPilotError("loaded random provider has trainable weights")
        ...
        validation_cache, validation_cache_manifest = load_normalized_parent_cache(
            ...
            expected_shape=config["upstream_screen"]["validation_cache"]["shape_n_t_y_x"],
        ...
        _heartbeat(partial, "pretraining_geometry_and_fixture_gate")
        ...
        model = decoder_module.FrozenLatentPixelBackgroundModel(
            provider.provider,
            decoder_initialization_seed=int(decoder_config["seed"]),
        ).to(requested_device)
        ...
        _atomic_torch_save(
            decoder_checkpoint_path,
            {
                ...
                "frozen_jepa_tensor_sha256": frozen_hash_before,
                "decoder_state_dict": {
        ...
        calibration_evidence = {
            "raw_source_off_calibration_pair_count": 0,
        ...
        for fixture in sorted(fixtures, key=lambda item: item.fixture_id):
            ...
            raw_scorer = FrozenNormalizedHandcraftedComparator(normalization)
            raw_row = evaluate_paired_fixture(
                fixture,
                raw_scorer,
                method=RAW_METHOD,
            ...
            if raw_scorer.fit_calls != 1 or raw_scorer.apply_calls != [
                "source_off",
                "source_on",
            ]:
                raise JEPAResidualPilotError("raw source-off calibration contract drifted")
            ...
            calibration_evidence["raw_source_off_calibration_pair_count"] += 1
            calibration_evidence["residual_source_off_calibration_pair_count"] += 2
            enriched_rows = []
            for row, units in (
                (raw_row, "frozen_training_normalized_observed_movie"),
        ...
        raw_anchor = raw_hc_cross_run_regression(
            _json_object(paths["paired_injection_results"]),
            rows,
        ...
        if not raw_anchor["exact_recovery_result_and_recall_identity"]:
            raise JEPAResidualPilotError(
        ...
        calibration_evidence.update(
            {
                "expected_raw_source_off_calibration_pair_count": len(fixtures),
