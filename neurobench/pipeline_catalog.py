...
_STAGE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "grid_128x128_generate": {
        ...
        "output": "grid_spec",
        "expected_qc_outputs": ("grid_overlay",),
        "why_use_it": "Creates deterministic 128x128 template-coordinate regions for max-pooled high-resolution model states.",
        "real_time_profile": {"mode": "offline", "latency_budget_ms": None},
    },
    "grid_state_extract": {
        "availability": "implemented",
        ...
        "why_use_it": "Pools registered frames into grid state sequences without detecting individual neurons; use max_intensity for 512-to-128 max-pooling.",
        "real_time_profile": {"mode": "offline", "latency_budget_ms": None},
    },
    "grid_dynamics_dataset_build": {
        "availability": "implemented",
...
_PARAMETER_DOCS: dict[str, dict[str, str]] = {
    ...
    "features": "Grid feature channels to extract, such as mean_intensity or max_intensity.",
    "pooling": "Optional pooling hint; max maps the default feature to max_intensity.",
    ...
    "temporal_stride_frames": "Temporal subsampling step applied before windowing.",
    "prediction_target": "Latent RNN target: absolute next code or delta from the last input code.",
...
STAGE_CATALOG: dict[str, PipelineStage] = {
    ...
    "grid_128x128_generate": PipelineStage(
        stage_id="grid_128x128_generate",
        label="Generate 128x128 grid",
        order=30,
        default_params={"rows": 128, "cols": 128, "bounds": "full_template_image", "cell_policy": "rectangular_image_coordinates"},
        param_ranges={"rows": ParameterRange(minimum=1, maximum=256), "cols": ParameterRange(minimum=1, maximum=256)},
        description="Create deterministic 128x128 rectangular regions in template coordinates.",
    ),
    ...
    "grid_dynamics_dataset_build": PipelineStage(
        ...
        order=70,
        default_params={"window_frames": 8, "prediction_horizon_frames": 1, "temporal_stride_frames": 1, "split_unit": "video", "split_method": "stratified_by_label", "train_fraction": 0.7, "val_fraction": 0.15, "test_fraction": 0.15},
        param_ranges={"window_frames": ParameterRange(minimum=1, maximum=1000), "prediction_horizon_frames": ParameterRange(minimum=1, maximum=1000), "temporal_stride_frames": ParameterRange(minimum=1, maximum=1000)},
    ...
    "grid_autoencoder_train": PipelineStage(
        ...
        order=75,
        default_params={"latent_dim": 32, "base_channels": 16, "epochs": 10, "batch_size": 32, "learning_rate": 0.001, "device": "cpu", "seed": 7},
        ...
        description="Train a small CNN autoencoder on grid frames.",
    ),
    "latent_rnn_train": PipelineStage(
        stage_id="latent_rnn_train",
        label="Train latent GRU predictor",
        order=80,
        default_params={"window_frames": 8, "hidden_dim": 64, "epochs": 10, "batch_size": 32, "learning_rate": 0.001, "prediction_target": "absolute", "device": "cpu", "seed": 7},
