...
## Current venue figures

The identity-safe JNM and Neuroinformatics drafts use the curated images in
[`venue/`](venue/), generated by
[`build_venue_manuscript_figures.py`](../scripts/build_venue_manuscript_figures.py).
The version-2 figure and repeated-fact manifests distinguish required repository
sources from `local_metadata_only` references to ignored `Outputs/` artifacts.
Both record SHA-256 hashes and byte sizes. A source-only checkout verifies every
committed source and rendered figure; a workstation with local evidence verifies
those additional inputs too. Full regeneration requires the retained local
sources. Metadata availability is not evidence release or scientific promotion.

## Earlier protected-run figure plan
