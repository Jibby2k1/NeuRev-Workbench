...
from neurobench.experiments.neuron_identifiability import jepa_residual_pilot as pilot
...
def test_maintained_config_loads_with_strict_screen_budget() -> None:
    config = pilot.load_jepa_residual_config(
        Path("examples/conditional_background_residual_v1.example.json")
    )
...
def test_preflight_rejects_every_scientific_or_claim_authorization(
    ...
    config = json.loads(
        Path("examples/conditional_background_residual_v1.example.json").read_text()
    )
    config[field] = True
