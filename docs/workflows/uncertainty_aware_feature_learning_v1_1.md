# Uncertainty-aware feature learning v1.1

Status: frozen implementation-only repair for bounded Run B.

Registry experiment: `NREV-EXP-0021`

## v1.1 implementation-only amendment

Run A (`NREV-RUN-EXP-0021-SCREEN-20260830-A`) failed incomplete after model
evaluation, before any model metric/result tables or final run summary were
serialized. Its `.partial` tree, source configuration, preflight, progress log,
and failure status remain immutable failure evidence. The failure was an
output-assembly schema bug: learned methods publish the alias `oof_scores`,
whereas the two frozen external scalar baselines publish `scores`; the paired-
bootstrap contrast assembler assumed the learned-method alias for every
comparator.

Version 1.1 changes only that implementation detail. Every downstream score
consumer now reads the common `oof_scores_fold_percentile` vector through one
fail-closed accessor and verifies exact agreement with the sole schema-specific
alias (`oof_scores` for learned methods or `scores` for external scalar
baselines). Raw fold-specific outputs receive the analogous exact-one-of alias
check. Run B has a new configuration, protocol hash, runner hash, run ID, and
non-colliding output root. Candidate, label, exclusion, feature, fold, solver,
seed, bootstrap, permutation, resource, audit, advance-signal, and claim
semantics below are unchanged from v1.
